# Community Survival Platform Docs

This planning set turns the platform vision into a concrete starting package.

Documents:

- [12-Step Success Plan](./12-step-success-plan.md)
- [90-Day Roadmap](./90-day-roadmap.md)
- [Planning System](./planning-system.md)
- [Current Focus](./current-focus.md)
- [Idea Inbox](./idea-inbox.md)
- [Repo Layout](./repo-layout.md)
- [Getting Started](./getting-started.md)

Starter docs:

- [Product Brief](./product-brief.md)
- [Architecture Principles](./architecture-principles.md)
- [MVP Scope](./mvp-scope.md)
- [Events](./events.md)
- [Progression DSL](./progression-dsl.md)
- [Vertical Slice Intent](./vertical-slice.md)
- [Research Telemetry v0](./research-telemetry-v0.md)

ADRs:

- [ADR 0001: Thin Client Platform](./adrs/0001-thin-client-platform.md)
- [ADR 0002: Edge Nodes Assist But Do Not Own Truth](./adrs/0002-edge-nodes-assist-but-do-not-own-truth.md)
- [ADR 0003: Multi-Lane Transport Strategy](./adrs/0003-websocket-transport.md)
- [ADR 0004: PostgreSQL Event Log](./adrs/0004-postgresql-event-log.md)
- [ADR 0005: .NET as Authoritative Backend Runtime](./adrs/0005-dotnet-authoritative-backend-runtime.md)
- [ADR 0012: Binary Payload Serialization](./adrs/0012-binary-payload-serialization.md)
- [ADR 0013: Dual-Channel UDP Transport](./adrs/0013-dual-channel-udp-transport.md)

Templates:

- [Sprint Template](./templates/sprint-template.md)
- [ADR Template](./templates/adr-template.md)
- [Idea Template](./templates/idea-template.md)

Recommended reading order:

1. Read the 12-step plan to understand scope and operating principles.
2. Read the 90-day roadmap to see milestones, owners, and delivery cadence.
3. Read the planning system to understand how ideas move into execution.
4. Read current focus before starting work.
5. Use the templates to run the next sprint and capture new ideas.
